...
**这里注意一下该写法只支持 `vue-loader at least v13.0.0+`理由 [pax-admin/issues/231](https://github.com/PAXFE/pax-admin/issues/231)**
...
`@/views/下的 .vue` 文件都会被打包。不管你是否被依赖。所以这样就产生了一个副作用，就是会多打包一些可能永远都用不到 js 代码。当然这只会增加 dist 文件的大小，但不会对线上代码产生任何的副作用。[相关 issue](https://github.com/PAXFE/pax-admin/issues/292)
...
[相关代码改动](https://github.com/PAXFE/pax-admin/pull/727)

## vue-cli@3

`pax-admin@4` 在新版本中已修改为基于 `vue-cli`来进行构建。所以在新版本中你只要在`.env.development`环境变量配置文件中设置`VUE_CLI_BABEL_TRANSPILE_MODULES:true`就可以了，具体[代码](https://github.com/PAXFE/pax-admin/blob/master/.env.development)。
