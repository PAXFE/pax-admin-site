...
::: tip 对应代码
[@/layout](https://github.com/PAXFE/pax-admin/tree/master/src/layout)
:::
...
`pax-admin` 中大部分页面都是基于这个 `layout` 的，除了个别页面如：`login` , `404`, `401` 等页面没有使用该`layout`。如果你想在一个项目中有多种不同的`layout`也是很方便的，只要在一级路由那里选择不同的`layout`组件就行。
...
示例代码：[@/views/example](https://github.com/PAXFE/pax-admin/tree/master/src/views/example)
:::
