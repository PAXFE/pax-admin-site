...
如果需要自定义构建，比如指定 `dist` 目录等，则需要通过 [config](https://github.com/PAXFE/pax-admin/blob/master/vue.config.js)的 `outputDir` 进行配置。

<br>

## 环境变量

所有测试环境或者正式环境变量的配置都在 [.env.development](https://github.com/PAXFE/pax-admin/blob/master/.env.development)等 `.env.xxxx`文件中。
...
pax-admin 中，前端路由使用的是 `vue-router`，所以你可以选择两种方式：`browserHistory` 和 `hashHistory`。
...
相关[issue](https://github.com/PAXFE/pax-admin/issues/370)
